...
win = pg.GraphicsLayoutWidget(title="Live Range-Doppler Viewer")
win.resize(800, 800)
win.show()
...
for i in range(2):
    for j in range(2):
        ...
        img = pg.ImageItem()
        p.addItem(img)
        p.setTitle(f"Angle{i*2 + j}")
    
        p.getAxis('bottom').setTicks([angle_ticks])
        p.getAxis('bottom').setLabel(f"Angle (degrees)")
...
def update():
    for fi in range(2):
        ...
        # plt.plot(adc_data[50, 1])
        # plt.plot(adc_data[100, 1])
        # plt.show()
        ...
        for i, (n, angle) in enumerate(zip(num_ant, angles)):
            ...
            channel_pad = (88, 89) if angle=='Elevation' else (82, 83)
            range_doppler_padded = np.pad(range_doppler_mean, pad_width=[(0, 0), channel_pad, (0, 0)], mode='constant')
            print("range_doppler_padded", range_doppler_padded.shape)
            ...
            beamformed_img = beamformed_img[beamformed_img.shape[0]//2:, :]
            beamformed_img = 10*np.log10(beamformed_img)[::2, :]
            print("beamformed_img", beamformed_img.shape)

            images[i][0].setImage(np.fliplr(beamformed_img.transpose()))
            # ax[i][0].set_title(f"Range {angle} Power Spectrum")
            # ax[i][0].set_xlabel(f"{angle} Angle (degrees)")
            # ax[i][0].set_ylabel("Range (meters)")
            # ax[i][0].set_yticks(range_ticks_real, range_tick_labels_real)
            # ax[i][0].set_xticks(np.linspace(0, 181, 9), np.linspace(-90, 90, 9).round(1))
            ...
            images[i][1].setImage(np.fliplr(range_azimuth[::2, :].transpose()))
            # ax[i][1].set_title(f"Range {angle} Power Spectrum")
            # ax[i][1].set_xlabel(f"{angle} Angle (degrees)")
            # ax[i][1].set_ylabel("Range (meters)")
            # ax[i][1].set_yticks(range_ticks_real, range_tick_labels_real)
            # ax[i][1].set_xticks(np.linspace(0, 181, 9), np.linspace(-90, 90, 9).round(1))
